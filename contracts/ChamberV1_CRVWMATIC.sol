...
contract ChamberV1_CRVWMATIC is
    IChamberV1,
    Ownable,
    ...
    AutomationCompatibleInterface
{
    ...
    function _burn(uint256 _shares) private {
...

        {
            ...
            if (token0Remainder > 0) {
                swapToken0ToStable(token0Remainder);
            }
            if (token1Remainder > 0) {
                swapToken1ToStable(token1Remainder);
            }
        }
    }
    ...
    function _repayAndWithdraw(
        ...
        if (token0OwnedByUser < token0DebtToCover + token0Swapped) {
            usdcSwapped += swapStableToExactToken0(
    ...
    function swapToken1ToStable(
        uint256 amountIn
    ) private returns (uint256) {
        uint256 amountOut = i_uniswapSwapRouter.exactInput(
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(i_token1Address, uint24(3000), i_token0Address,  uint24(500), i_usdcAddress),
    ...
    function swapToken0ToStable(
        uint256 amountIn
    ) private returns (uint256) {
        uint256 amountOut = i_uniswapSwapRouter.exactInput(
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(i_token0Address, uint24(500), i_usdcAddress),
                recipient: address(this),
                deadline: block.timestamp,
                amountIn: amountIn,
                amountOutMinimum: 0
    ...
    function swapStableToExactToken0(
        uint256 amountOut
    ) private returns (uint256) {
