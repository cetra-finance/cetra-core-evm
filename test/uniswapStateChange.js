...
describe("Basic tests new", function () {
    ...
    it("should chande slot0 states", async function () {
        ...
        let newTickHex = ethers.utils.hexlify(
            parseInt("0x9B000000", 16) + newTick
        );
        ...
        await helpers.setStorageAt(
            UniPool.address,
            0,
            prevSlot0Hex.slice(0, 16) + newTickHex + prevSlot0Hex.slice(26, 66)
        );
        console.log("slot0", await UniPool.slot0());
    });

    it("should change fee states", async function () {
        let newHexFeeGrowthGlobal0X128 = ethers.utils.hexlify(
            newFeeGrowthGlobal0X128
        );
        let zero0 = 66 - newHexFeeGrowthGlobal0X128.length;
        newHexFeeGrowthGlobal0X128 =
            "0x" + "0".repeat(zero0) + newHexFeeGrowthGlobal0X128.slice(2);

        let newHexFeeGrowthGlobal1X128 = ethers.utils.hexlify(
            newFeeGrowthGlobal1X128
        );
        let zero1 = 66 - newHexFeeGrowthGlobal1X128.length;
        newHexFeeGrowthGlobal1X128 =
            "0x" + "0".repeat(zero1) + newHexFeeGrowthGlobal1X128.slice(2);
        ...
        console.log(
            ...
            await UniPool.feeGrowthGlobal0X128()
        );
        console.log(
            "feeGrowthGlobal1X128",
            await UniPool.feeGrowthGlobal1X128()
        );
    });
});
