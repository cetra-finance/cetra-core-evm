...
describe("linkToEthPriceConst", function () {
    let owner, _, user1, user2, donorWallet;
    let usd, wmatic, aUSD, vLINK, vWMATIC;
        ...
        if (way) {
            await UniRouter.connect(user).exactInput({
                path: ethers.utils.solidityPack(
                    ...
                    [
                        networkConfig[network.config.chainId + 3].usdcAddress,
                        3000,
                        networkConfig[network.config.chainId + 3].wmaticAddress,
                        3000,
                        networkConfig[network.config.chainId + 3].linkAddress,
                    ]
                ),
    ...
    const makeAllChecks = async () => {
        ...
        console.log(
            "link:",
            (await link.balanceOf(chamber.address)).toString()
        );
        ...
        console.log(
            "DEPT IN LINK:",
            (await vLINK.balanceOf(chamber.address)).toString(),
            "\nDEPT IN WMATIC:",
            (await vWMATIC.balanceOf(chamber.address)).toString()
        );
    ...
    before(async () => {
        ...
        vLINK = await ethers.getContractAt(
            "IERC20",
            currNetworkConfig.aaveVLINKAddress
        )
        vWMATIC = await ethers.getContractAt(
            "IERC20",
        ...
        UniRouter = await ethers.getContractAt(
            "ISwapRouter",
            networkConfig[network.config.chainId + 3].uniswapRouterAddress
        );

        const chamberFactory = await ethers.getContractFactory("ChamberV1_LINKETH");
        ...
        await helpers.impersonateAccount(networkConfig[network.config.chainId].donorWalletAddress);
        donorWalletH = await ethers.getSigner(
            networkConfig[network.config.chainId].donorWalletAddress
        );

        await usd
            .connect(donorWalletH)
        ...
        await chamber
            .connect(owner)
            .giveApprove(usd.address, currNetworkConfig.aaveVWMATICAddress);
        ...
        await chamber
            .connect(owner)
            .giveApprove(wmatic.address, currNetworkConfig.uniswapRouterAddress);
        await chamber
            .connect(owner)
            .giveApprove(wmatic.address, currNetworkConfig.uniswapPoolAddress);
        await chamber
            .connect(owner)
            .giveApprove(wmatic.address, currNetworkConfig.aaveVWMATICAddress);

        await chamber
            .connect(owner)
            .giveApprove(link.address, currNetworkConfig.aaveV3PoolAddress);
        await chamber
            .connect(owner)
            .giveApprove(
                link.address,
    ...
    describe("should make swaps in uni pools, so our position collect some fees", async function () {
        let WmaticLinkPrices, WmaticUsdcPrices;

        it("makes all swaps", async function () {

            console.log("usd/link", await getPriceFromPair(link, usd, 3000, 1e18, 1e6))
            console.log("wmatic/usd", await getPriceFromPair(usd, wmatic, 3000, 1e6, 1e18))
            console.log("link/wmatic", await getPriceFromPair(wmatic, link, 3000, 1e18, 1e18))

            for (let i = 0; i < 10; i++) {
                await makeSwap(donorWalletH, 40000, true);
                await makeSwap(donorWalletH, 5000, false);
            }

            WmaticLinkPrices = await getPriceFromPair(
                wmatic,
                link,
                3000,
                1e18,
            ...
            WmaticUsdcPrices = await getPriceFromPair(usd, wmatic, 3000, 1e6, 1e18);
            const linkTargetPrice =
                Math.round((WmaticLinkPrices[1]/ WmaticUsdcPrices[1]) * 1e8) *
                1e10;

                if (
                    (await getPriceFromPair(link, usd, 3000, 1e18, 1e6))[1] *
                        1e18 <
                    BigNumber.from(linkTargetPrice.toString())
                ) {
                    while (
                        (await getPriceFromPair(link, usd, 3000, 1e18, 1e6))[1] *
                            1e18 <
                        ...
                        makeSwapHelper(donorWalletH, 4000, true);
                    }
                } else {
                    while (
                        (await getPriceFromPair(link, usd, 3000, 1e18, 1e6))[1] *
                        ...
                        makeSwapHelper(donorWallet, 500, false); 
                    }
                }    
        ...
        it("should set all oracles", async function () {
            await setNewOraclePrice(wmatic, Math.round(WmaticUsdcPrices[1] * 1e8));
            await setNewOraclePrice(
                link,
                Math.round((WmaticUsdcPrices[1] / WmaticLinkPrices[1]) * 1e8)
            );
        });
    });
