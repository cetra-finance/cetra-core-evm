...
describe("Basic tests newCrv", function () {
            ...
            await UniRouter.connect(user).exactInput({
                path: ethers.utils.solidityPack(
                    ...
                    [
                        ...
                        500,
                        networkConfig[network.config.chainId].crvAddress,
                        500,
    ...
    describe("should make swaps in uni pools, so our position collect some fees", async function () {
        let CrvWmaticPrices, CrvUsdcPrices;
        ...
        it("makes all swaps", async function () {
            ...
            console.log(
                "usd/crv",
                await getPriceFromPair(crv, usd, 500, 1e18, 1e6)
            );
            console.log(
                "matic/crv",
                await getPriceFromPair(crv, wmatic, 500, 1e18, 1e18)
            );
            ...
            CrvWmaticPrices = await getPriceFromPair(
                crv,
                wmatic,
                500,
                1e18,
            ...
            CrvUsdcPrices = await getPriceFromPair(crv, usd, 500, 1e18, 1e6);
            const wmaticTargetPrice =
                Math.round((CrvUsdcPrices[1] / CrvWmaticPrices[1]) * 1e8) *
                1e10;
        ...
        it("should set all oracles", async function () {
            await setNewOraclePrice(crv, Math.round(CrvUsdcPrices[1] * 1e8));
