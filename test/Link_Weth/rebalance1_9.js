...
describe("linkToEthPriceConst", function () {
    let owner, _, user1, user2, donorWallet, donorWalletH;
    let usd, weth, aUSD, vLINK, vWETH;
        ...
        if (way) {
            await UniRouter.connect(user).exactInput({
                path: ethers.utils.solidityPack(
                    ...
                    [
                        networkConfig[network.config.chainId + 2].usdcAddress,
                        3000,
                        networkConfig[network.config.chainId + 2].wethAddress,
                        3000,
                        networkConfig[network.config.chainId + 2].linkAddress,
                    ]
                ),
    ...
    before(async () => {
        ...
        UniRouter = await ethers.getContractAt(
            "ISwapRouter",
            networkConfig[network.config.chainId + 2].uniswapRouterAddress
        );
    ...
    describe("should make swaps in uni pools, so our position collect some fees", async function () {
        let WethLinkPrices, WethUsdcPrices;

        it("makes all swaps", async function () {

            console.log("usd/link", await getPriceFromPair(link, usd, 3000, 1e18, 1e6))
            console.log("usd/weth", await getPriceFromPair(weth, usd, 3000, 1e18, 1e6))
            console.log("link/weth", await getPriceFromPair(weth, link, 3000, 1e18, 1e18))

            for (let i = 0; i < 10; i++) {
                await makeSwap(donorWallet, 60000, true);
                await makeSwap(donorWallet, 5000, false);
            }

            WethLinkPrices = await getPriceFromPair(
                weth,
                link,
                3000,
                1e18,
            ...
            WethUsdcPrices = await getPriceFromPair(weth, usd, 3000, 1e18, 1e6);
            const linkTargetPrice =
                Math.round((WethUsdcPrices[1] / WethLinkPrices[1]) * 1e8) *
                1e10;
